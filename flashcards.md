...
<style>
	...
	@media (max-width: 768px) {
		...
		.arrow_btn { width: 30px; height: 30px; font-size: 14px; }
		.shuffle_btn,
		.fullscreen_btn { width: 30px; height: 30px; font-size: 12px; margin-left: 4px; }
		.flashcard_wrapper { gap: 11px; }
		.card_row { gap: 11px; }
	}

	@media (orientation: landscape) and (max-height: 500px) and (pointer: coarse) {
		.flashcard_wrapper { gap: 17px; }
		.card_row { gap: 17px; }
	}
	...
	body.study_fullscreen #flashcards_content {
		...
		z-index: 9000;
		overflow: hidden;
		padding: 0 max(28px, env(safe-area-inset-right)) 28px max(28px, env(safe-area-inset-left));
	...
	body.study_fullscreen .fullscreen_spacer_below { display: none; }
	...
	body.study_fullscreen .card_content { font-size: 46px; padding: 40px; }
	body.study_fullscreen .card_counter { font-size: 30px; }
	...
	@media (orientation: landscape) and (max-height: 500px) and (pointer: coarse) {
		...
		body.study_fullscreen .flashcard_wrapper { justify-content: flex-start; }
		body.study_fullscreen .flashcard_container {
			max-width: min(92vw, calc((100dvh - 100px) * 2));
			max-height: min(calc((100dvh - 100px), 46vw));
		}
		...
		body.study_fullscreen .fullscreen_btn { width: 40px; height: 40px; font-size: 18px; margin-left: 10px; }
		body.study_fullscreen #flashcards_content {
			padding: 0;
			padding-top: env(safe-area-inset-top);
...
<div id = "flashcards_content">
	...
	<div class = "mode_selector">
		<button class = "mode_btn active" data-mode = "study">
			study
		</button>
		<button class = "mode_btn" data-mode = "quiz">
			quiz
		</button>
	</div>
	...
	<div id = "study_mode" class = "mode_content active">
		<div class = "controls">
			<select id = "study_deck_select">
				<option value = "">-- select a deck --</option>
			</select>
			<button id = "start_study_btn" disabled>
				start
			</button>
		</div>
						...
						<div class = "flashcard" id = "study_card">
							<div class = "card_front">
								<div class = "card_counter">
									<span id = "current_card_num">1</span>/<span id = "total_cards">0</span>
								</div>
								<div class = "card_content" id = "study_front"></div>
							</div>
							<div class = "card_back" style = "display:none;">
								<div class = "card_counter">
									<span id = "current_card_num_back">1</span>/<span id = "total_cards_back">0</span>
								</div>
	...
	<div id = "quiz_mode" class = "mode_content">
		<div class = "controls">
			<select id = "quiz_deck_select">
				<option value = "">-- select a deck --</option>
			</select>
			<button id = "start_quiz_btn" disabled>
				start
			</button>
		</div>
		...
		<div id = "quiz_results" style = "display:none;">
			<p>
				score = <span id = "final_score">0</span>/<span id = "final_total">0</span> (<span id = "final_percentage">0</span>%).
			</p>
			<button id = "retake_quiz_btn">
				retake
			</button>
		</div>
	</div>
